...
class Model(nn.Module):
    ...
    def __init__(self, configs, N=512, N2=32):
        ...
        self.modes1 = min(configs.mode_Num,self.pred_len//2)
        #self.modes1 = 32
        ...
        if self.configs.ab == 2:
            ...
            self.spec_conv_1 = nn.ModuleList([SpectralConv1d(in_channels=n, out_channels=n,seq_len=min(self.pred_len,self.seq_len), modes1=configs.mode_Num,compression=0,ratio=configs.ratio,mode_type=self.mode_type) for n in self.window_size for _ in range(len(self.multiscale))])               
            self.mlp = nn.Linear(len(self.multiscale)*len(self.window_size), 1)
        
        self.linear = nn.ModuleList([nn.Linear(n, n) for n in self.window_size])
